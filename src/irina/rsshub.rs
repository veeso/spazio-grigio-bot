...
impl RssHubClient {
    ...
    pub async fn get_latest_post() -> anyhow::Result<Entry> {
        ...
        match feed.entries().next() {
            Some(e) => Ok(e.clone()),
            None => anyhow::bail!("Ciao sono Irina. Il mio percorso nel minimalismo mi ha portato ad avere un account instagram senza alcun post."),
        }
    }
    ...
    async fn get_posts() -> anyhow::Result<Feed> {
        ...
        Client::new(config.rsshub_url)
            .fetch_picuki(SPAZIOGRIGIO_ACCOUNT_ID)
            .await
